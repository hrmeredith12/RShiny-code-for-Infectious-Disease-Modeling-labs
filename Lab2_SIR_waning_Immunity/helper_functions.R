# Helper functions for rShiny App for exploring parameters of a SIR Model with births, deaths, and waning immunity
# By Hannah Meredith
...
sir <- function(time, state, parameters) {
  ...
  with(as.list(c(state, parameters)), {
    ...
    dS <- mu * N - beta * S * I / N               - mu * S + omega * R    ## Change in Susceptibles         
    dI <-          beta * S * I / N  - gamma * I  - mu * I                ## Change in Infected
    dR <-                              gamma * I  - mu * R - omega * R    ## Change in Recovered
...
generate.data.frames <- function(beta, gamma, mu, omega, time.dur){
  ### Set parameters
  init       <- c(S = 9999, I = 1, R = 0, C = 1)              ## Counts in each compartment
  parameters <- c(beta = beta, gamma = gamma, mu = mu, omega = omega, time.dur = time.dur)   ## beta: infection parameter; gamma: recovery parameter
...
SIR.table <- function(beta, gamma, mu, omega, time.dur){
  ## Import datasets for making table
  sir.data <- generate.data.frames(beta, gamma, mu, omega, time.dur)
  out <- sir.data[[2]]
...
SIR.plots <- function(beta, gamma, mu, omega, time.dur){
  ...
  epi.curve <- ggplot(daily.cases, aes(day, daily.cases))+
    ...
    # xlim(min = 0, max = t_end)+
    # geom_text(aes(label=floor(new.cases)), vjust=-0.3, size=3.5)+
    theme(
      axis.title = element_text(size = 20),
      axis.text = element_text(size = 18),
      legend.title = element_text(size = 20),
...
test <- SIR.table(beta = 0.21,
                  gamma = 1/14,
                  time.dur = 1000,
                  mu = (1/76)/365,
                  omega = (1/1)/365)

test.plot <- SIR.plots(beta = 0.21,
                  gamma = 1/14,
                  time.dur = 1500,
                  mu = (1/76)/365,
                  omega = (1/1)/365)
